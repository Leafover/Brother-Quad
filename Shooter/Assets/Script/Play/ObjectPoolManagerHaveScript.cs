...
public class ObjectPoolManagerHaveScript : MonoBehaviour
{
    ...
    public ItemBase gunItemPrefab, itemHealthPrefab, itemCoinPrefab, itemItemDropPrefab;
    ...
    public void Init()
    {
        ...
        if (itemItemDropPooler == null)
        {
            go = new GameObject("itemItemDropPooler");
            itemItemDropPooler = go.AddComponent<ObjectPoolerHaveScript>();
            itemItemDropPooler.itemPooledObject = itemItemDropPrefab;
            go.transform.parent = this.gameObject.transform;
            itemItemDropPooler.InitializeItem(3);
            AllPool.Add(itemItemDropPooler);
        }
    }
    GameObject go;
}
