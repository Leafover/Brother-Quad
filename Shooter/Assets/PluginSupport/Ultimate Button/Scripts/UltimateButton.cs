...
*/
[ExecuteInEditMode]
public class UltimateButton : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IDragHandler
{
	/* ----- > ASSIGNED VARIABLES < ----- */
	...
	public AnchorSide anchorSide = AnchorSide.Top;
	...
	IEnumerator WaitForPositioning()
	{
		...
		switch( positioning )
		{
			...
			case Positioning.RelativeToJoystick:
			{
				UpdateSizeAndPlacementRelativeToJoystick();
			}
			break;
			case Positioning.RelativeToButton:
			{
				UpdateSizeAndPlacementRelativeToButton();
			}
			break;
		}
	}
	...
	public Vector4 GetSpaceButton()
	{
		...
		Vector2 imagePosition1 = ConfigureImagePosition(tempVector, new Vector2(0, 0));
		Vector2 imagePosition2 = ConfigureImagePosition(tempVector, new Vector2(50, 100));
		var space = new Vector4();
		space.x = imagePosition2.y > imagePosition1.y ? imagePosition2.y : imagePosition1.y;
		space.y = imagePosition2.y > imagePosition1.y ? imagePosition1.y : imagePosition2.y;
		space.z = imagePosition1.x > imagePosition2.x ? imagePosition2.x : imagePosition1.x;
		space.w = imagePosition1.x > imagePosition2.x ? imagePosition1.x : imagePosition2.x;
		return space;
	}
	...
	public Vector2 GetCustomSpacing(Vector2 pos)
	{
		...
		float textureSize = referenceSize * (buttonSize / 10);
		//Vector2 tempVector = new Vector2(textureSize, textureSize);
		//baseTrans.position = pos - ((baseTrans.sizeDelta - tempVector) / 2);
		//sizeFolder.position = pos;
		...
		Vector2 customSpacing;
		customSpacing.y = pos.y / (Screen.height - textureSize) * 100;
		customSpacing.x = (anchor == Anchor.Left ? pos.x : ((Screen.width - textureSize) - pos.x)) / (Screen.width - textureSize) * 100;
	...
	void UpdateSizeAndPlacementRelativeToButton ()
	{
		if( rectTrans == null || targetButtonBase == null || sizeFolder == null )
			return;

		// Configure a size for the image based on the Canvas's size and scale.
		float textureSize = rectTrans.sizeDelta.x * ( buttonSize / 5 );
		...
		Vector2 pos = targetButtonBase.position;

		if( anchorSide == AnchorSide.Top )
		{
			pos.x -= tempVector.x / 2;
			pos.y += rectTrans.sizeDelta.y / 2;
		}
		else if( anchorSide == AnchorSide.Bottom )
		{
			pos.x -= tempVector.x / 2;
			pos.y -= rectTrans.sizeDelta.y / 2 + tempVector.y;
		}
		else if( anchorSide == AnchorSide.Right )
		{
			pos.x += rectTrans.sizeDelta.x / 2;
			pos.y -= tempVector.y / 2;
		}
		else if( anchorSide == AnchorSide.Left )
		{
			pos.x -= rectTrans.sizeDelta.x / 2 + tempVector.x;
			pos.y -= tempVector.y / 2;
		}

		{
			...
			float relativeSpacingX = customSpacing_X;
			if( anchorSide == AnchorSide.Left )
				relativeSpacingX = ( relativeSpacingX - 100 ) * -1;

			float relativeSpacingY = customSpacing_Y;
			if( anchorSide == AnchorSide.Bottom )
				relativeSpacingY = ( relativeSpacingY - 100 ) * -1;

			space.x = rectTrans.sizeDelta.x * ( ( relativeSpacingX - 50 ) / 100 );
			space.y = rectTrans.sizeDelta.y * ( ( relativeSpacingY - 50 ) / 100 );
	...
	void UpdateSizeAndPlacementRelativeToJoystick ()
	{
		if( joystick == null || sizeFolder == null )
			return;

		// Configure a size for the image based on the Canvas's size and scale.
		float textureSize = joystick.joystickSizeFolder.sizeDelta.x * ( buttonSize / 5 );
		...
		Vector2 pos = joystick.joystick.position;

		if( anchorSide == AnchorSide.Top )
		{
			pos.x -= tempVector.x / 2;
			pos.y += joystick.joystickSizeFolder.sizeDelta.y / 2;
		}
		else if( anchorSide == AnchorSide.Bottom )
		{
			pos.x -= tempVector.x / 2;
			pos.y -= joystick.joystickSizeFolder.sizeDelta.y / 2 + tempVector.y;
		}
		else if( anchorSide == AnchorSide.Right )
		{
			pos.x += joystick.joystickSizeFolder.sizeDelta.x / 2;
			pos.y -= tempVector.y / 2;
		}
		else if( anchorSide == AnchorSide.Left )
		{
			pos.x -= joystick.joystickSizeFolder.sizeDelta.x / 2 + tempVector.x;
			pos.y -= tempVector.y / 2;
		}

		{
			...
			space.x = joystick.joystickSizeFolder.sizeDelta.x * ( ( relativeSpacingX - 50 ) / 100 );
			space.y = joystick.joystickSizeFolder.sizeDelta.y * ( ( relativeSpacingY - 50 ) / 100 );
	...
	public void UpdatePositioning ()
	{
		...
		else if( positioning == Positioning.RelativeToJoystick )
			UpdateSizeAndPlacementRelativeToJoystick();
		else if( positioning == Positioning.RelativeToButton )
			UpdateSizeAndPlacementRelativeToButton();
	}
